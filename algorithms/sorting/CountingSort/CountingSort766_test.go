...
// TestCountingSort766 tests the behavior of the countingSort function under various scenarios.
func TestCountingSort766(t *testing.T) {
	tests := []struct {
		name     string
		input    []int
		expected []int
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer func() {
				if r := recover(); r != nil && !tc.wantErr {
					t.Errorf("Test failed with panic for input %v, did not expect panic", tc.input)
				}
			}()

			result := countingSort(tc.input)
			if (result == nil && !tc.wantErr) || (result != nil && !equal(result, tc.expected)) {
...
// largeArray generates a large array for performance testing.
func largeArray() []int {
	var arr []int
	for i := 10000; i >= 1; i-- {
		arr = append(arr, i)
...
// largeArraySorted generates a sorted large array for performance testing.
func largeArraySorted() []int {
	var arr []int
	for i := 1; i <= 10000; i++ {
		arr = append(arr, i)
	}
...
// equal checks if two slices of integers are the same.
