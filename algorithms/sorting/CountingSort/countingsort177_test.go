...
func Testcountingsort177(t *testing.T) {
	tests := []testData{
		{
			description: "Test countingSort with a sorted input array",
			input:       []int{1, 2, 3, 4, 5},
			expected:    []int{1, 2, 3, 4, 5},
		},
		{
			description: "Test countingSort with a reverse-sorted input array",
			input:       []int{5, 4, 3, 2, 1},
			expected:    []int{1, 2, 3, 4, 5},
		},
		{
			description: "Test countingSort with a large input array",
			input:       []int{1000, 500, 2000, 1500, 3000},
			expected:    []int{500, 1000, 1500, 2000, 3000},
		},
		{
			description: "Test countingSort with an empty input array",
			input:       []int{},
			expected:    []int{},
		},
		{
			description: "Test countingSort with an array containing duplicate elements",
			input:       []int{2, 3, 2, 1, 3},
			expected:    []int{1, 2, 2, 3, 3},
		},
		{
			description: "Test countingSort with a single-element input array",
			input:       []int{5},
			expected:    []int{5},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			if !reflect.DeepEqual(result, test.expected) {
				t.Errorf("Test Failed: %s - Expected: %v, Got: %v", test.description, test.expected, result)
			} else {
				t.Logf("Test Passed: %s", test.description)
			}
		})
	}
}
